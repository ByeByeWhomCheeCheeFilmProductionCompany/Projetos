...
foreach ($canal as $line){
    ...
    echo '<div class="col-lg-12  '.$line ->slug.'"><a href="'.$linkPeraWebCat.'"><h1 class="page-header secao text-capitalize">'.$line ->slug.'</h1></div><div class="row "></a>';

    $return = json_decode(file_get_contents('http://noticias.peraweb.com.br/json-to-rss/jsons/canal-'.$line ->slug.'.json'));;
...
echo '</pre>-->';
    foreach ($secao->users as $k =>$line){
        ...
        $linkPeraWebSec = 'http://noticias.peraweb.com.br/'.$slug_geral.'/'.$line->screen_name;

        $linkPeraWeb = 'http://noticias.peraweb.com.br/'.$slug_geral.'/'.$line->screen_name.'/'.$line->status->id.'/'.removeAcentos(preg_replace("/(http:\/\/)(.*?)\/([\w\.\/\&\=\?\-\,\:\;\#\_\~\%\+]*)/", "",$line->status->text),'-');
                                ...
                                <div class="thumbnail" style="overflow:hidden;padding:0;">
                                    ...
                                    echo '
                                        ...
                                        <div class="text-center">
                                            ...
                                            <a class="btn btn-social-icon btn-facebook" rel="prettyPhoto[iframes]" title="Google.com opened at 100%" href="https://www.facebook.com/sharer/sharer.php?u=http://noticias.peraweb.com.br/'.$slug_geral.'/'.$line->screen_name.'/'.$line->status->id.'/'.$line->status->text.'&iframe=true&width=500&height=250" target="_blank"><i class="fa fa-facebook"></i></a>

                                            <a class="btn btn-social-icon btn-google-plus" rel="prettyPhoto[iframes]" title="Google.com opened at 100%" href="https://plus.google.com/share?url=http://noticias.peraweb.com.br/'.$slug_geral.'/'.$line->screen_name.'/'.$line->status->id.'/'.$line->status->text.'&iframe=true&width=500&height=250" target="_blank"><i class="fa fa-google-plus"></i></a>

                                            <a class="btn btn-social-icon btn-twitter" rel="prettyPhoto[iframes]" title="Google.com opened at 100%" href="https://twitter.com/intent/tweet?text=http://noticias.peraweb.com.br/'.$slug_geral.'/'.$line->screen_name.'/'.$line->status->id.'/'.$line->status->text.'&iframe=true&width=500&height=250" target="_blank"><i class="fa fa-twitter"></i></a>
